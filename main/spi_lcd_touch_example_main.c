...
#endif
#include "../managed_components/lvgl__lvgl/src/core/lv_disp.h"
...
extern void cubic_demo(lv_disp_t *disp);
...
{
    xEventGroupSetBits(my_event_group, MENUWORK);
}

void lv_gui_start(void)
{
    static lv_style_t *startbtn_style;
    lv_style_init(&startbtn_style);
    lv_style_set_bg_color(&startbtn_style, lv_color_hex(0x1E90FF));

    static lv_style_t *startmenu_tile_style;
    lv_style_init(&startmenu_tile_style);
    lv_style_set_text_font(&startmenu_tile_style, &lv_font_montserrat_14);

    start_src = lv_obj_create(lv_scr_act());
    lv_obj_set_style_bg_color(start_src, lv_color_hex(0x003a57), LV_PART_MAIN);
    lv_obj_set_size(start_src, 320, 240);

    lv_obj_t *title = lv_label_create(start_src);
    lv_label_set_text(title, "Welcome to use ESP32C3 Terminal");
    lv_obj_add_style(title, &startmenu_tile_style, LV_PART_MAIN);
    lv_obj_set_style_text_color(start_src, lv_color_hex(0xffffff), LV_PART_MAIN);
    lv_obj_align(title, LV_ALIGN_TOP_MID, 0, 0);

    lv_obj_t *startbtn = lv_btn_create(start_src);
    lv_obj_set_size(startbtn, LV_PCT(50), 50);
    lv_obj_align(startbtn, LV_ALIGN_BOTTOM_MID, 0, 0);

    lv_obj_t *label = lv_label_create(startbtn);
    lv_label_set_text(label, "ENTRY");
    lv_obj_align(label, LV_ALIGN_CENTER, 0, 0);
    lv_obj_add_event_cb(startbtn, startbtn_event_cb, LV_EVENT_CLICKED, NULL);
}
static void menu_task(void *pvParameters)
{
    xEventGroupWaitBits(my_event_group, MENUWORK, pdFALSE, pdFALSE, portMAX_DELAY);
    menu();
